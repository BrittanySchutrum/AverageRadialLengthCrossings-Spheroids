...
% Imports ROI into Matlab, calculates radial length to each perimeter point and creates the  [radialdistance_section] vector, plots the perimeter
% points of the ImageJ ROI with centroid in MATLAB coordinates (figure 1)
% by default the pixel dimentions are defined as 2048x2048. The number does
% not matter unless your ROI is bigger than these dimensions in pixels 


intersectionnumber=plot_intersections(radiallengths_section,excursion_length,minimum_distance);
% Plots the radial lengths and determines sustained crossings defined by
% the excursion length and minimum crossing distance(figure 2)

%Printed Quantitative Outputs = 1) number of Average Radial Length Crossing (ARLC)
%and 2) the Standard Deviation of the Radial Length (SDRL)
...
%       centroid.
    ...
    % Figure 1 is a plot of perimeter points with centroid
    figure;
    hold on;
    scatter(xcoords, ycoords, 1, "black", DisplayName="Perimeter Points");
    line(xcoords, ycoords,'Color','black', 'DisplayName','Perimeter')
    scatter(xcoords(section), ycoords(section), 15, 'r', 'filled', ...
        'DisplayName', sprintf('Selected sector (%.1f°–%.1f°)', ...
        rad2deg(theta_start), rad2deg(theta_end)));
    plot(centroidx, centroidy, 'k+', 'MarkerSize', 10, 'DisplayName', 'Centroid');
    legend;
    axis equal;
    title(sprintf('ROI plot ') + string(name)+ sprintf('Selected sector (%.1f°–%.1f°)', ...
        rad2deg(theta_start), rad2deg(theta_end)));
...
function [intersectionnumber] = plot_intersections(rad_dist, exlength, mindist)
...
%   Linear interpolation to find inflection points between discrete perimeter points.
    ...
    % Figure 2 is a plot of the radial distances, average radial distance, and marks of the inflection points.
    figure
    plot(1:length(rad_dist),rad_dist,'-','LineWidth', 2,'Color','#ffa62b','DisplayName', 'Radial Lengths');
    set(gca, 'FontName', 'Arial', 'FontSize', 12);
    set(gcf, 'renderer', 'painters');
    hold on
    grid on
    yline(mean(rad_dist),'--','LineWidth',2,'Color','#ee2e31','DisplayName','Average Radial Length');
    format_title = sprintf("ARLC = %d (Excursion length = %d, Minimum distance = %d)",intersectionnumber, exlength, mindist);
    title(format_title);
    legend({'Radial Lengths','Average Radial Length'},'Location','southwest','EdgeColor','#05299e');
    plot(inflection_points, ones(1, length(inflection_indexes))*mean(rad_dist), 'ko', 'MarkerSize', 5, 'MarkerEdgeColor', 'black', 'MarkerFaceColor', '#b892ff' ,'DisplayName', 'Crossings');
    xlabel("Perimeter Point")
    ylabel("Radial Distance (pixels)")

     % printing to command window    
    fprintf('Average Radial Length Crossings: %d\n', length(inflection_points));
    fprintf('Standard Deviation of Radial Lengths: %d\n', std(rad_dist))
end
